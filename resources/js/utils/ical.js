...
export function icalLogic() {
    return {
        ...
        async fetchAuthenticatedUser() {
            debugLog('icalLogic: fetchAuthenticatedUser called');
            try {
                ...
                debugError('icalLogic: Error in fetchAuthenticatedUser (should be handled in app.js):', error);
        ...
        copyIcalUrl() {
            debugLog('icalLogic: copyIcalUrl called');
            ...
            if (urlElement && this.userIcalUrl && this.userIcalUrl !== 'ログインすると表示されます。' && this.userIcalUrl !== 'エラーにより取得できませんでした。') {
                ...
                navigator.clipboard.writeText(urlElement.innerText)
                    .then(() => {
                        debugLog('icalLogic: URL copied successfully.');
                        // showToastNotification を app.js のコンテキストから呼び出す想定
                        if (typeof this.showToastNotification === 'function') { // showToastNotification が存在するか確認
                    ...
                    .catch(err => {
                        debugError('icalLogic: Failed to copy URL: ', err);
                        // showToastNotification を app.js のコンテキストから呼び出す想定
                        if (typeof this.showToastNotification === 'function') { // showToastNotification が存在するか確認
                ...
                debugWarn('icalLogic: No iCal URL to copy.');
                // showToastNotification を app.js のコンテキストから呼び出す想定
                if (typeof this.showToastNotification === 'function') { // showToastNotification が存在するか確認
